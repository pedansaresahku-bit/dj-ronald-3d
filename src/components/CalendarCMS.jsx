...
export default function CalendarCMS({
  ...
  return (
    <section id="kalender" className="py-20 relative z-10 bg-[#060507]">
      <div className="w-[94%] max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 border-b border-white/10 pb-6">
          ...
          <div className="flex items-center gap-3 self-start lg:self-auto">
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-white/5 border border-white/15 text-gray-300 flex items-center justify-center hover:bg-white/10 hover:text-white hover:border-amber-400/50 transition-all"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>

            {/* Current Month & Year Display Pill */}
            <div className="px-5 py-2.5 rounded-full bg-[#12131a] border border-white/15 flex items-center gap-2.5 shadow-inner">
              ...
              {currentYear === 2026 && currentMonth === 8 && (
                <span className="px-2 py-0.5 rounded text-[10px] font-mono font-black bg-amber-400 text-black tracking-wider uppercase">
                  NOW
                </span>
              )}
            </div>
            ...
            {onOpenCMS && (
              <button
                ...
                className="w-10 h-10 ml-2 rounded-full bg-cyan/10 border border-cyan/30 text-cyan flex items-center justify-center hover:bg-cyan hover:text-black transition-all"
              >
            ...
            if (hasEvent) {
              ...
              return (
                <motion.div
                  ...
                  className={`relative p-3.5 sm:p-4 rounded-2xl flex flex-col justify-between h-[165px] sm:h-[175px] cursor-pointer transition-all duration-300 select-none group ${
                    dayItem.isToday
                      ? 'bg-gradient-to-b from-[#18160f] to-[#0c0d12] border-[1.5px] border-amber-400 shadow-[0_0_20px_rgba(250,204,21,0.25)]'
                      : 'bg-[#0d0e14] border-[1.5px] border-amber-400/80 hover:border-amber-400 hover:shadow-[0_0_20px_rgba(250,204,21,0.2)]'
                  }`}
                >
                  {/* Card Header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      {dayItem.isToday && (
                        <span className="ml-1.5 px-1.5 py-0.5 rounded text-[9px] font-mono font-black bg-amber-400 text-black flex items-center gap-0.5 uppercase shadow-sm">
                  ...
                  <div className="my-auto py-1">
                    <div className="text-[10px] font-mono font-bold text-amber-400 tracking-wider flex items-center gap-1 uppercase truncate">
                      <MapPin className="w-3 h-3 text-amber-400 shrink-0" />
                      <span>{ev.city || 'INDONESIA'}</span>
                    </div>

                    <h4 className="font-display font-extrabold text-xs sm:text-sm text-white tracking-wide truncate mt-0.5 group-hover:text-amber-300 transition-colors uppercase">
                  ...
                  <div className="flex items-center justify-between pt-1 border-t border-white/5">
                    ...
                    <button
                      ...
                      className="text-[10px] font-mono font-bold text-amber-400 hover:text-amber-300 flex items-center gap-0.5 transition-colors uppercase"
                    >
                      <span>DETAIL</span>
              ...
              return (
                <div
                  key={dayItem.dateKey}
                  onClick={() => handleCardClick(dayItem)}
                  className="p-3.5 sm:p-4 rounded-2xl bg-[#09090e]/70 border border-white/5 hover:border-white/20 transition-all flex flex-col justify-between h-[165px] sm:h-[175px] cursor-pointer group"
                >
                  {/* Card Header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                  ...
                  {/* Card Footer */}
                  <div className="flex items-center justify-between pt-1 border-t border-white/5">
                    <span className="text-[10px] font-mono text-gray-400 group-hover:text-amber-400 transition-colors uppercase tracking-wider">
                      INQUIRE
                    </span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-5 mt-10 pt-6 border-t border-white/10">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm font-light text-gray-400">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(250,204,21,0.8)] shrink-0" />
            <span>
          ...
          <button
            ...
            className="w-full sm:w-auto px-7 py-3 rounded-full bg-amber-400 hover:bg-amber-300 text-black font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-400/25 transition-all hover:scale-105 active:scale-95 shrink-0"
          >
            REQUEST EVENT DATE
          </button>
        ...
        {selectedEvent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              transition={{ duration: 0.2 }}
              className="relative w-full max-w-2xl rounded-3xl bg-[#0e0f17] border border-amber-400/50 shadow-2xl overflow-hidden"
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedEvent(null)}
                className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/60 text-white flex items-center justify-center hover:bg-amber-400 hover:text-black transition-all"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="p-6 sm:p-8">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2.5 py-1 rounded-full text-[11px] font-mono font-bold bg-amber-400/10 text-amber-400 border border-amber-400/30 uppercase flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="flex flex-col justify-between gap-3 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                    ...
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-mono font-bold text-gray-200 border border-white/10 hover:border-amber-400/40 transition-all"
                    >
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  <button
                    ...
                    className="w-full sm:flex-1 py-3 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-400/20 transition-all flex items-center justify-center gap-2"
                  >
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full py-3.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-400/20 transition-all flex items-center justify-center gap-2"
                >
