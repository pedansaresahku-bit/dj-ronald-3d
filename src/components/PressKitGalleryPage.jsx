...
export default function PressKitGalleryPage({ onOpenLightbox, onBackToHome }) {
  ...
  return (
    <div className="min-h-screen bg-obsidian text-white pt-24 pb-20 selection:bg-cyan selection:text-black">
      ...
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-cyan/15 rounded-full blur-[140px] pointer-events-none z-0" />
      <div className="fixed bottom-0 right-0 w-[400px] h-[300px] bg-purple/15 rounded-full blur-[120px] pointer-events-none z-0" />
      ...
      <div className="w-[92%] max-w-[1600px] mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 pb-6 border-b border-white/10">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 hover:border-cyan text-xs font-mono font-bold text-white transition-all w-fit group"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className="px-4 py-2 rounded-full bg-cyan hover:bg-cyan/90 text-black font-display font-black text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-lg shadow-cyan/30 transition-all hover:scale-105"
            >
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan/15 border border-cyan/40 text-cyan text-xs font-mono font-bold uppercase mb-4 shadow-md shadow-cyan/15"
          >
          ...
          >
            Press Kit <span className="bg-gradient-to-r from-white via-cyan to-purple bg-clip-text text-transparent drop-shadow-[0_0_25px_rgba(0,240,255,0.4)]">Galery</span>
          </motion.h1>
          ...
          {GALLERY_CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-mono font-bold tracking-wide transition-all ${
                selectedCat === cat.id
                  ? 'bg-cyan text-black font-black shadow-lg shadow-cyan/30 scale-105'
                  : 'bg-white/10 border border-white/15 text-slate-200 hover:text-white hover:bg-white/20'
              }`}
            >
              {cat.label} ({cat.count})
            </button>
          ...
          <AnimatePresence>
            {filteredPhotos.map((photo, idx) => (
              <motion.div
                ...
                className="group relative rounded-3xl overflow-hidden bg-obsidian-surface border border-white/15 hover:border-cyan hover:shadow-[0_0_25px_rgba(0,240,255,0.3)] transition-all flex flex-col justify-between aspect-[3/4] cursor-pointer"
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent opacity-75 group-hover:opacity-90 transition-opacity" />
                ...
                <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between z-10">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-black/70 backdrop-blur-md border border-white/20 text-white hover:bg-cyan hover:text-black flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                    title="Inspect Photo"
                  >
                ...
                <div className="absolute bottom-0 inset-x-0 p-4 z-10 translate-y-2 group-hover:translate-y-0 transition-transform">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-white/15 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="mt-16 p-8 rounded-3xl bg-obsidian-surface border border-white/15 text-center flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onBackToHome}
              className="px-6 py-3 rounded-full bg-white/10 hover:bg-white/20 text-white font-mono text-xs font-bold border border-white/20 transition-all"
            >
              Kembali ke Home
            </button>
            <a
              href="/#epk-rider"
              onClick={onBackToHome}
              className="px-6 py-3 rounded-full bg-cyan hover:bg-cyan/90 text-black font-display font-black text-xs uppercase tracking-wider shadow-lg shadow-cyan/30 transition-all"
            >
              Buka EPK Rider
            </a>
          </div>
